...
function calcularGrupos() {
    ...
    if (cupo > 70) {
        alert('El cupo máximo por grupo no puede ser mayor a 70 estudiantes.');
        return;
    }

    const gruposNecesarios = Math.ceil(cantidad / cupo);
    let postulantesRestantes = cantidad;
    let distribucionHtml = '';

    for (let i = 1; i <= gruposNecesarios; i++) {
        const estudiantesGrupo = Math.min(postulantesRestantes, cupo);
        distribucionHtml += `<p class="mb-1"><strong>Grupo ${i}:</strong> ${estudiantesGrupo} estudiante${estudiantesGrupo === 1 ? '' : 's'}</p>`;
        postulantesRestantes -= estudiantesGrupo;
    }
    ...
    document.getElementById('resultado').innerHTML = `
        <div class="alert alert-success" role="alert">
            ...
            <hr>
            <p class="mb-2"><strong>Distribucion:</strong></p>
            ${distribucionHtml}
        </div>
    `;
}
