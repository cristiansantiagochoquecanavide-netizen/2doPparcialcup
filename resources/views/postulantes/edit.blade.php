        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('postulantes.update', $postulante->id_postulante) }}" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="id_carrera_opcion_primera" class="form-label">
                            <i class="bi bi-mortarboard"></i> Carrera 1ª Opción
                        </label>
                        <select class="form-select @error('id_carrera_opcion_primera') is-invalid @enderror" 
                                id="id_carrera_opcion_primera" name="id_carrera_opcion_primera">
                            <option value="">-- Seleccionar --</option>
                            @foreach($carreras as $carrera)
                                <option value="{{ $carrera->id_carrera }}" @selected(old('id_carrera_opcion_primera', $postulante->id_carrera_opcion_primera) == $carrera->id_carrera)>
                                    {{ $carrera->nombre }}
                                </option>
                            @endforeach
                        </select>
                        @error('id_carrera_opcion_primera')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

                    <div class="mb-3">
                        <label for="id_carrera_opcion_segunda" class="form-label">
                            <i class="bi bi-mortarboard"></i> Carrera 2ª Opción
                        </label>
                        <select class="form-select" id="id_carrera_opcion_segunda" name="id_carrera_opcion_segunda">
                            <option value="">-- Seleccionar --</option>
                            @foreach($carreras as $carrera)
                                <option value="{{ $carrera->id_carrera }}" @selected(old('id_carrera_opcion_segunda', $postulante->id_carrera_opcion_segunda) == $carrera->id_carrera)>
                                    {{ $carrera->nombre }}
                                </option>
                            @endforeach
                        </select>
